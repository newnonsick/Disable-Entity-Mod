...
public final class DisableEntityKeybindManager {
    ...
    private static final KeyBinding TOGGLE_ALL = register("key.disable_entity.toggle_all");
    private static final KeyBinding TOGGLE_ENTITIES = register("key.disable_entity.toggle_entities");
    private static final KeyBinding TOGGLE_PARTICLES = register("key.disable_entity.toggle_particles");
    private static final KeyBinding TOGGLE_BLOCK_ENTITIES = register("key.disable_entity.toggle_block_entities");
    private static final KeyBinding TOGGLE_NAMETAGS = register("key.disable_entity.toggle_nametags");
    private static final KeyBinding TOGGLE_BLOCK_STATES = register("key.disable_entity.toggle_block_states");
    private static final KeyBinding TOGGLE_WORLD_RENDERING = register("key.disable_entity.toggle_world_rendering");
    private static final KeyBinding RESET_DEFAULTS = register("key.disable_entity.reset_defaults");
    private static final KeyBinding COPY_CONFIG = register("key.disable_entity.copy_config");
    private static final KeyBinding PASTE_CONFIG = register("key.disable_entity.paste_config");
    ...
    public static void register() {
        ...
        ClientTickEvents.END_CLIENT_TICK.register(DisableEntityKeybindManager::onClientTick);
        registered = true;
    }
    ...
    private static void onClientTick(MinecraftClient client) {
        ...
        while (TOGGLE_ALL.wasPressed()) {
            ...
            announce(client,
                    enabled ? "message.disable_entity.global_enabled" : "message.disable_entity.global_disabled");
        }
        while (TOGGLE_ENTITIES.wasPressed()) {
            boolean enabled = DisableEntityConfigManager.toggleEntityRendering();
            announce(client,
                    enabled ? "message.disable_entity.entities_enabled" : "message.disable_entity.entities_disabled");
        }
        while (TOGGLE_PARTICLES.wasPressed()) {
            boolean enabled = DisableEntityConfigManager.toggleParticles();
            announce(client,
                    enabled ? "message.disable_entity.particles_enabled" : "message.disable_entity.particles_disabled");
        }
        while (TOGGLE_BLOCK_ENTITIES.wasPressed()) {
            boolean enabled = DisableEntityConfigManager.toggleBlockEntities();
            announce(client, enabled ? "message.disable_entity.block_entities_enabled"
        ...
        while (TOGGLE_NAMETAGS.wasPressed()) {
            ...
            announce(client,
                    enabled ? "message.disable_entity.nametags_enabled" : "message.disable_entity.nametags_disabled");
        }
        while (TOGGLE_BLOCK_STATES.wasPressed()) {
            boolean enabled = DisableEntityConfigManager.toggleBlockStates();
            announce(client, enabled ? "message.disable_entity.block_states_enabled"
        ...
        while (TOGGLE_WORLD_RENDERING.wasPressed()) {
            ...
            announce(client, enabled ? "message.disable_entity.world_rendering_enabled"
    ...
    private static void copyConfigToClipboard(MinecraftClient client) {
        ...
        GLFW.glfwSetClipboardString(windowHandle, DisableEntityConfigManager.exportConfigToJson());
    }
    ...
    private static void pasteConfigFromClipboard(MinecraftClient client) {
        ...
        boolean imported = DisableEntityConfigManager.importConfigFromJson(clipboardText);
        announce(client,
                imported ? "message.disable_entity.config_imported" : "message.disable_entity.config_import_failed");
    }

    private static void announce(MinecraftClient client, String translationKey) {
        client.inGameHud.setOverlayMessage(Text.translatable(translationKey), false);
